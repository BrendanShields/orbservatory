...
export function computeSessionStats(input: StatsInput): SessionStatsBase {
  ...
  // Providers without the optional stats surface degrade to event-derived stats, marked partial below.
  const usageByModel = normalizer.usageByModel ?? new Map<string, TokenTotals>();
  for (const [model, t] of usageByModel) {
  ...
  let subagentCount = 0;
  const agents = normalizer.getAgents ? normalizer.getAgents() : normalizer.snapshot([]).agents;
  ...
  const stats: SessionStatsBase = {
    ...
    retries,
    errors,
    userTurns: normalizer.userTurns ?? 0,
    durationMs,
    models: [...usageByModel.keys()].filter((m) => m !== 'unknown'),
    firstActive,
    lastActive: firstActive ? firstActive + durationMs : (normalizer.lastActiveTs ?? 0),
  };
  if ((normalizer.parseFailures ?? 0) > 0 || !normalizer.usageByModel) stats.partial = true;
  return stats;
}
